...
/** Meer beschikbare velden + historie = iets hoger gewicht */
...
export function fuseStationReadings(readings: StationReading[]): FusedLiveWind | null {
  ...
  const speedMs =
    contributors.reduce((s, c) => s + c.speedMs * c.weight, 0) / totalWeight;
  const gustMs =
    contributors.reduce((s, c) => s + c.gustMs * c.weight, 0) / totalWeight;
  ...
  const ageMinutes = observationAgeMinutes({ value: speedMs, timestamp: observationTimestamp }) ?? 0;
