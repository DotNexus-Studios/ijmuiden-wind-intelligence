...
import { getForecastPreviewData } from "@/lib/dashboard";
...
export async function GET(request: Request) {
  try {
    ...
    const data = await getForecastPreviewData(riderWeight);
    return NextResponse.json(data, {
      headers: { "Cache-Control": "public, s-maxage=300, stale-while-revalidate=60" },
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Voorspelling ophalen mislukt";
